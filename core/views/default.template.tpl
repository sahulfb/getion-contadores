    ...
    <header class="encabezado">
        ...
        <div class="sub-encabezado d-md-block d-none">
            <div class="optiones-a">
                {foreach from=$templateMenus item=menu}
                    {if $menu['con_opciones'] === FALSE}
                    <a class="opcion-a" href="{$menu['link']}">
                        <i class="mr-1 {$menu['icon']}"></i> {$menu['label']}
                    </a>
                    {else}
                    <div class="opcion-a">
                        <div class="content-a dropdown-toggle" data-toggle="collapse" data-target="#collapse-{$menu['slug']}">
                            <i class="mr-1 {$menu['icon']}"></i> {$menu['label']}
                        </div>

                        <div class="opciones-b collapse" id="collapse-{$menu['slug']}">
                            {foreach from=$menu['opciones'] item=opcion}
                                {if $opcion['con_opciones'] === FALSE}
                                    <a href="{$opcion['link']}" class="opcion-b border-bottom">
                                        <i class="mr-1 {$opcion['icon']}"></i> {$opcion['label']}
                                    </a>
                                {else}
                                    <div class="border-bottom">
                                        <div class="opcion-b dropdown-toggle" data-toggle="collapse" data-target="#collapse-{$opcion['slug']}">
                                            <i class="mr-1 {$opcion['icon']}"></i> {$opcion['label']}
                                        </div>
                                        <div class="opciones-c collapse" id="collapse-{$opcion['slug']}">
                                            {foreach from=$opcion['opciones'] item=subopciones}
                                                <a class="opcion-c" href="{$subopciones['link']}">
                                                    <i class="mr-1 {$subopciones['icon']}"></i> {$subopciones['label']}
                                                </a>
                                            {/foreach}
                                        </div>
                                    </div>
        ...
        <div class="row flex-xl-nowrap" menu-lateral="false">
            ...
            <div class="border-right menu-lateral bg-light d-block d-md-none" id="menu-lateral">
                ...
                <div class="lista-menu" id="group-menu">
                    ...
                    {foreach from=$menuLateral item=menus key=seccion}
                        <div class="lista-menu" id="group-menu">
                            <div class="menu-separator text-white" style="background-color: #4a4a4a;">
